...
% - what happens if you beat the last level? confetti or sum

%CURRENT BUGS/PROBLEMS
% - block spawns at edge of screen sometimes (too far to side)...cannot
% find an issue with the formula, but I'll address it if it happens again
% - game doesn't know what to do once player loses (must add a variable or
% use an existing one to tell computer how to continue forward once player
% loses all lives)
...
if lvl == 1
    ...
    pause(0.5)
    lvl_num_p = patch(lvl_1X+44.5,lvl_1Y-15,white);
else %change lvl number
    pause(0.5)
    delete(lvl_num_p)
    if lvl == 2
...
elseif lives == 0 %death is by shift, delete all remaining hearts
    for n = 1:10
        for H = 1:lives
        delete(hearts(H))
        new_heartX = heartX*(11-n)/10;
