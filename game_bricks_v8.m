...
%DEBUGGING NOTES
% - minimum y-position of bottom of brick grid is 1.25; so far this value
%   does not seem to have any significance
% - potential issue: shift cannot happen while ball is within grid (can
% create issues, clearly)...has not seemed to affect me yet, but keep in
% mind
% - still do not know why shift only occurs when ball touches block...
...
auto_on = 0; %represents whether auto mode is on or not
...
while stop ~= 1
    ...
    %Check for and update "shift limit"
    %if brickCornerYPos(numBricksDown,1) <= 0 %lowest brick row is below y=0
    %disp('yeauh')
    %   [bricks,canShift] = deleteBottomRow(bricks,bricksStatus,...
    %numBricksAcross,numBricksDown);
    %end
    ...
    if BallPos(2)-BallRad-BlockThickness < 0
        if abs(BallPos(1) - BlockCenter) < bwidth/2
           ...
           if canShift == 1 %bricks can be shifted
            ...
            Strong,Boom,brickXCoords,brickYCoords,brickCornerXPos,...
...
%ALMOST POSITIVE THE FOLLOWING FUNCTION ISNT NEEDED
function [bricks,canShift] = deleteBottomRow(bricks,bricksStatus,...
    numBricksAcross,numBricksDown)
ok = 1;
...
if ok == 1
    canShift = 1; %entire bottom row doesn't exist, shift can be done
    numBricksDown = numBricksDown-1; %change numBricksDown
    %Redefine bricks array excluding the bottom row
    for r = 1:numBricksDown
        for c = 1:numBricksAcross
            bricksNew(r,c) = bricks(r,c);
        end
    end
    %Use bricksNew array value in place of the old bricks array value
    delete(bricks)
    bricks = bricksNew;
else
    canShift = 0; %brick(s) on bottom row still exist, shift cannot be done
end

%Check for existence of entire row
%while existence == 0 && n <= numBricksAcross
%    n = n+1;
%    existence = bricksStatus(numBricksDown,n);
%end
%Check to see how many bricks on bottom row do not exist
%if n == numBricksAcross
%    canShift = 1; %entire bottom row doesn't exist, shift can be done
%    numBricksDown = numBricksDown-1; %change numBricksDown
    %Redefine bricks array excluding the bottom row
%    for r = 1:numBricksDown
