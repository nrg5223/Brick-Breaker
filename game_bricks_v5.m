...
dshift = -0.5; %change in shift after each block hit
numBricksAcross = 7;
numBricksDown = 5;
...
crackedbrickXCoords = [0,0.65*brickWidth,0.6*brickWidth,0.8*brickWidth,...
    0.7*brickWidth,0.8*brickWidth,brickWidth,brickWidth,...
    0.45*brickWidth,0.5*brickWidth,0.35*brickWidth,0.4*brickWidth,...
    0.3*brickWidth,0,0,0.1*brickWidth,0];
crackedbrickYCoords = [0,0,0.4*brickDepth,0.8*brickDepth,0.4*brickDepth,...
    0,0,brickDepth,brickDepth,0.8*brickDepth,0.6*brickDepth,...
    0.8*brickDepth,brickDepth,brickDepth,0.4*brickDepth,...
    0.3*brickDepth,0.2*brickDepth];
...
if BallPos(2) < 100-brickDepth*numBricksDown+shift %ball is beneath bricks
    ...
    if BallPos(2)+BallRad > maxY
        ...
        StrongCracked,Strong,Boom,crackedbrickXCoords,...
        crackedbrickYCoords,brickCornerXPos,brickCornerYPos);
    end
...
function [bricksType,bricksExist,bricks] = brickReact(numBricksDown,...
    ...
    crackedbrickYCoords,brickCornerXPos,brickCornerYPos)
if horSect == numBricksDown+1 %ball is below bricks
    if bricksType(numBricksDown,vertSect) == Basic ||...
       bricksType(numBricksDown,vertSect) == StrongCracked
        %Delete brick
        bricksExist(numBricksDown,vertSect) = 0;
        delete(bricks(numBricksDown,vertSect));
    elseif bricksType(numBricksDown,vertSect) == Strong
        %Crack brick and change brick type
        delete(bricks(numBricksDown,vertSect))
        bricks(numBricksDown,vertSect) = patch(crackedbrickXCoords...
            +brickCornerXPos(numBricksDown,vertSect),...
            crackedbrickYCoords+brickCornerYPos(numBricksDown,...
            vertSect),darkred);
            bricksType(numBricksDown,vertSect) = StrongCracked
    elseif bricksType(numBricksDown,vertSect) == Boom
    end
elseif horSect == 0 %ball is above bricks
else %ball is within bricks
end
