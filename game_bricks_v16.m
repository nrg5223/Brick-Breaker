...
%CURRENT BUG
% - lasermeter_p's don't delete if you lose a life
...
heartShift = -0.3; %change in y position of heart after each iteration
...
r = randi(numBricksDown);
c = randi(numBricksAcross);
LaserBrickPos = [r,c]; %save position of Laser Brick
...
r = randi(numBricksDown);
c = randi(numBricksAcross);
LifeBrickPos = [r,c]; %save position of Laser Brick
...
if BallPos(2) < 100-brickDepth*numBricksDown+shift %ball is beneath bricks
    ...
    if BallPos(2)+BallRad > maxY && yDirection == up
        BallVel(2) = -BallVel(2);
        change = inY;
        [bricksStatus,bricks,B_exploding,offx,offy,G_time,lasergun_on,shots_left,life_down] = brickReact(numBricksDown,...
        ...
        BlockCenter,BlockThickness,lightgray,lasergun_on,shots_left,Life,life_down);
        yDirection = down;
    end
...
%Lose heart animation
  for n = 1:10
    ...
    hearts(lives) = patch(new_heartX+minX+2/3*bThickness+heartdist*(lives-1),...
                          new_heartY-15,pink);
    pause(p)
end
...
countdown(white); %countdown before game starts again
...
function [bricksStatus,bricks,B_exploding,offx,offy,G_time,lasergun_on,shots_left,life_down] = brickReact(numBricksDown,...
        horSect,vertSect,bricksStatus,bricks,StrongBrickColor,Basic,...
        StrongCracked,Strong,Boom,crackedbrickXCoords,xDirection,...
        crackedbrickYCoords,brickCornerXPos,brickCornerYPos,yDirection,...
        up,down,left,right,change,inX,inY,Steel,brickWidth,brickDepth,...
...
xShift = 50;
yShift = 50;
pause(1) %second before animation starts (looks good like this)
%Three
